...
def verify_dataset_clinical_context(
    df: pd.DataFrame, score_threshold: float = 85.0
) -> bool:
    """
...
def hybrid_schema_extractor(
    ...
    for target_key, synonyms in CLINICAL_ONTOLOGY_MAP.items():
        ...
        if best_match_col:
            ...
            logger.info(
                f"🎯 Mapped Target [{target_key}] ➔  Column [{best_match_col}] (Confidence: {highest_score:.1f}%)"
            )
            # Remove mapped column to avoid double assignment across close indicators
            incoming_columns.remove(best_match_col)
        else:
            logger.warning(
                f"⚠️  Failed to match Target [{target_key}] to a fuzzy column. Closest match was '{matched_string}' (Confidence: {score:.1f}%)"
            )
    print()
