...
def enrich_with_rag(pre_report_content: str):
    ...
    query_results = collection.query(query_texts=[pre_report_content], n_results=1)

    # Extract the top matching medical text block
    retrieved_context = (
        query_results["documents"][0][0]
        if query_results["documents"]
        else "No matching protocol found."
    )
    ...
    logger.info(
        "🤖 HERMES Agent 2 is compiling enriched medical advisory guidelines..."
    )
