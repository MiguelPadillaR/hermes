...
def load_mimic_dataset() -> pd.DataFrame:
    ...
    """
    logger.info("Selected MIMIC-IV ICU dataset.")

    base_path = (
        DATASETS_DIR
        / "montassarba/mimic-iv-clinical-database-demo-2-2/mimic-iv-clinical-database-demo-2.2"
    )
    ...
    vitals_pivot["sbp"] = vitals_pivot["sbp_invasive"].fillna(
        vitals_pivot["sbp_non_invasive"]
    )
    vitals_pivot["dbp"] = vitals_pivot["dbp_invasive"].fillna(
        vitals_pivot["dbp_non_invasive"]
    )
    vitals_pivot["mbp"] = vitals_pivot["mbp_invasive"].fillna(
        vitals_pivot["mbp_non_invasive"]
    )
    ...
    df = (
        vitals_pivot.groupby(["subject_id", "hadm_id", "stay_id"])
...
def load_mock_dataset() -> pd.DataFrame:
    ...
    Returns:
        df (pd.DataFrame): Processed MIMIC dataframe.
    """
    logger.info("Selected MOCK dataset.")
